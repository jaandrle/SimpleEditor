...
module.exports= function({app, $gulp_folder, gulp, error, $g, $o, $run}){
    return function(cb){
        ...
        if($run.jshint_advanced){
            cmd= $o.spawn($run.jshint_advanced, [app.bin_folder], {});
        ...
        function run(code){
            let main_stream;
            if(!code){
                main_stream= gulp.src([app.bin_folder+"*.js", '!'+app.bin_folder+'*.sub.js'])
                    .pipe($g.replace(/\/\/gulp\.place\.(.*)/g,function(s, filename){return $o.fs.readFileSync(app.bin_folder+filename, 'utf8').replace(/[^\n]*\/\/gulp\.remove\.line/g, "");}))
                    .pipe($g.replace(/\/\/gulp\.place\.(.*)/g,function(s, filename){return $o.fs.readFileSync(app.bin_folder+filename, 'utf8').replace(/[^\n]*\/\/gulp\.remove\.line/g, "");}))
